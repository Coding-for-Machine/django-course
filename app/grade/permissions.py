...
class IsStaff(HttpBearer):
    """
    Xodim (staff) bo‘lgan foydalanuvchilarga ruxsat beradi.
    """
    def authenticate(self, request, token):
...
class IsSuperuser(HttpBearer):
    """
    Superuser bo‘lsa barcha API-larga kira oladi.
    """
    def authenticate(self, request, token):
...
class IsInGroup(HttpBearer):
    """
    Foydalanuvchi berilgan guruhda bormi-yo‘qligini tekshiradi.
    """
